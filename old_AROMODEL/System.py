...
class System(object):
    ...
    def Update_Atom_Positions(self,File_Name):
        ...
        for line in lines:
            if len(line.split()) > 0 and line.split()[0] == 'Atoms':
                Read_Flag = True
            elif Read_Flag and len(line.split()) == 7:
    ...
    def Write_Plumed_Files(self,Write_Flags,Base_File_Name,Fit_Energies,Force_y,Force_x,Delocalization_Modifier=1.0,Nonbonded_Modifier=1.0,Non_Interacting=False,Reverse=False,Folder_Name="./Full_Polymer_Plumed"):
        ...
        for flag,mol in zip(Write_Flags,self.Molecule_List):
            ...
            if flag:
                ...
                for i in range(0,len(mol.Ring_List)-1):
                    ...
                    torsion_file.write("\n\nc%d: CENTER ATOMS=%d" % (index*2-3, mol.Ring_List[i+1].Plumed_Rings[0][0].System_ID)) #Writes the first atom in the first plumed ring
                    for atom in mol.Ring_List[i+1].Plumed_Rings[0][1:]:
                        torsion_file.write(",%d" % atom.System_ID)
                    torsion_file.write("\n\nc%d: CENTER ATOMS=%d" % (index*2-2, mol.Ring_List[i].Plumed_Rings[1][0].System_ID)) #Writes the first atom in the second plumed ring
                    for atom in mol.Ring_List[i].Plumed_Rings[1][1:]:
                        torsion_file.write(",%d" % atom.System_ID)
                    ...
                    torsion_file.write("\n\nc%d_normal: GHOST ATOMS=c%d,%d,%d COORDINATES=0.0,1.0,0.0\n\n" % (index*2-3,index*2,mol.Ring_List[i].Plumed_Rings[plumed_index][0].System_ID,mol.Ring_List[i].Plumed_Rings[plumed_index][1].System_ID))
                    torsion_file.write("\n\nc%d_normal: GHOST ATOMS=c%d,%d,%d COORDINATES=0.0,1.0,0.0\n\n" % (index*2-2,index*2+1,mol.Ring_List[i].Plumed_Rings[plumed_index][0].System_ID,mol.Ring_List[i].Plumed_Rings[plumed_index][1].System_ID))
                    plumed_index += 1
                    if plumed_index >= 2:
                        plumed_index = 0
                    if Reverse:
                        try:
                            ...
                            torsion_file.write("DIH_%d: TORSION VECTOR1=c%d_normal,c%d AXIS=%d,%d VECTOR2=c%d_normal,c%d\n\n" % (index - 1, index - 1, index - 1, 0, 0, index, index))
                    else:
                        try:
                            ...
                            torsion_file.write("DIH_%d: TORSION VECTOR1=c%d,c%d_normal AXIS=%d,%d VECTOR2=c%d,c%d_normal\n\n" % (index - 1, index - 1, index - 1, 0, 1, index, index))
