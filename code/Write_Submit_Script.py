...
def Write_Orca(f,nproc,In_File,Name):
...
# 	return
def Write_SLURM(File_Name,In_File,Job_Name,nproc,Cluster_Location,Job_Type,proc_per_node=28,walltime = 2,Executable_Path = "",OMP_Path = "",constraint='knl'):	
	""" 
	...
	Cluster_Location: 
	Job_Type: string, 
	proc_per_node: no clue TODO
	walltime: beats me TODO
	Executable_Path: 
	OMP_Path:
	constraint:
...
def Write_SLURM_Batch(File_Name,In_File_List,Name,nproc,Cluster_Location,Job_Type,queue="premium",proc_per_node=32,walltime = 2,constraint = 'haswell',nodes = 1, account = "ddp381"):

	print("Writing slurm batch: File_Name: %s,\n Name: %s,\n In_File_List: %s,\n Job_Type: %s"%(File_Name,Name,In_File_List,Job_Type))
	
	print("I'm opening %s! THis is in Write_SLURM_BATCH"%File_Name) #TODO: temporary, delete later
	f = open(File_Name,'w')
	"""nodes = math.floor(nproc/proc_per_node)
	if nodes == 0:
		nodes = 1
	...
	if Job_Type == "Orca":
		for In_File in In_File_List:
			Write_Orca(f,nproc,In_File,Name)
